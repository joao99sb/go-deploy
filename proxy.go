...
type Proxy struct {
	CurrentBackendHost string
	CurrentUrl         *url.URL
	proxyChangeSignal  chan string

	peerMap map[Peer]int
}

func NewProxy() *Proxy {
	return &Proxy{
		proxyChangeSignal: make(chan string),
		peerMap:           make(map[Peer]int),
	}
}
...
func (p *Proxy) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if r.Header.Get("Upgrade") == "websocket" {
		NewWebsocketProxy(p.CurrentUrl)
		return
	}
	...
	reverseProxy := httputil.NewSingleHostReverseProxy(p.CurrentUrl)
...
func (p *Proxy) setProxyURL(addr string) {
	urlString := fmt.Sprintf("http://0.0.0.0:%s", addr)
	...
	p.CurrentUrl = u
	p.CurrentBackendHost = fmt.Sprintf(":%s", addr)
}
...
// NewPWebsocketProxy returns a new Websocket reverse proxy that rewrites the
